...
void setup(void) {
  ...
  setupString = "Starting...running setup() ";
...
bool findAzimuthServo()
{
  ...
  if (nMotes > 0)
  {
    ...
    for (i=0; i<nMotes; i++)
    {
      SerialMon.print(F("...searching mote: ")); SerialMon.print(motes[0]);
      SerialMon.println(F(" for directional servos"));
      ...
      if (ChariotEP.coapSearchResources(motes[i], azimuthSearchName, response))
      {
        if (ChariotEP.is_205_CONTENT(response))
        {
          ...
          servoMote = motes[i];
          Serial.print(F("Servo mote is "));
          Serial.println(servoMote);
          displayMsg = "Servo mote is: " + servoMote + ".";
          displayStatusMsg(displayMsg);
          return true;
        }
    ...
    SerialMon.println(F("No azimuth servo motes found."));
    displayMsg = "No azimuth servo motes found.";
    displayStatusMsg(displayMsg);
  } 
  else 
  {
    SerialMon.println(F("Currently no motes visible."));
    displayMsg = "Currently no motes visible.";
    displayStatusMsg(displayMsg);
  }
