...
#elif defined(HAVE_HWSERIAL3) || defined(__SAM3X8E__)
	// MEGA/DUE Host
    #define LEONARDO_HOST   0
    #define UNO_HOST    	0
...
class ChariotEPClass
{
  public:
	...
	bool processSerialInput(int milliseconds);
	...
	inline bool is_205_CONTENT(String& response) {
	  int x;
	  if ((x = response.indexOf("2.05 CONTENT ")) != -1)
	  {
		return true;
	...
	inline bool strip_204_CHANGED(String& response) {
	  int x;
	  if ((x = response.indexOf("2.04 CHANGED ")) != -1)
	  {
		response.remove(0, x+13);
		return true;
	  }
	...
	inline bool is_204_CHANGED(String& response) {
	  int x;
	  if ((x = response.indexOf("2.04 CHANGED ")) != -1)
	  {
		return true;
