...
bool ChariotEPClass::triggerResourceEvent(int handle, String& eventVal, bool signalChariot)
{
	...
	while ((iterations < 100) && (ch != '\r')) {
		...
		if (ChariotClient.available()) {
			...
			delay(10);  //GHW changed from '1' on 10/1/16
		}
	}
	...
	delay(1000); // Give signal generator on Chariot a delay.
	return true;
}
...
void ChariotEPClass::process() 
{
...
#if EP_DEBUG
	SerialMon.print(F("Process--incoming msg: "));
	SerialMon.println(command);
#endif
  ...
  SerialMon.println(param);
		...
		{
			String *Str;
			if ((Str = putCallbacks[id](param)) != NULL && (Str->length() > 0))
			{

#if EP_DEBUG
  SerialMon.println(F("PUT: sending triggered event now."));
#endif

				delay(500); // This delay separates the response to PUT from the event signal.
		...
		String result = "4.02 CMD NOT UNDERSTOOD.\n\0";
		ChariotClient.print(result);
#if EP_DEBUG
...
/* search named mote for resource */
...
void ChariotEPClass::serialChariotCmd()
{
  ...
  while (Serial.available() && !terminator_seen) {
  ...
  if  ((chariotLclCmd == "motes") || (chariotLclCmd == "hosts") || (chariotLclCmd == "health") || (chariotLclCmd == "session-id") ||
		(chariotLclCmd == "root/set") || (chariotLclCmd == "root/get") || (chariotLclCmd == "radio/on") ||
		(chariotLclCmd == "radio/off") || (chariotLclCmd == "serial/clear") || (chariotLclCmd == "serial/status"))
  { 
	Cmd = "sys/";
	Cmd += chariotLclCmd;
	Cmd += "\n\0";
...
/*
 * Wrapper that is called from the sketch to take care of Serial cmds.
 */
bool ChariotEPClass::processSerialInput()
{
  if (Serial.available()) {
    serialChariotCmd();
	return true;
 ...
 * Wrapper that is called from the sketch to take care of Serial cmds.
 */
bool ChariotEPClass::processSerialInput(int milliseconds)
{
  /*
   * delay time must be in [25, 100];
   */
...
void ChariotEPClass::serialChariotCmdHelp()
{
	...
	SerialMon.println(F("session-id -- get session id for this Chariot system session."));
...
bool ChariotEPClass::localChariotCmd(String& command, String& response)
{
  ...
  if ((command == "motes") || (command == "hosts") || (command == "health") || (command == "session-id") ||
		(command == "root/set") || (command == "root/get") || (command == "radio/on") ||
		(command == "radio/off") || (command == "serial/clear") || (command == "serial/status"))
  { 
	Cmd = "sys/";
	Cmd += command;
	Cmd += "\n\0";
...
/*
 * Get remote mote's session id.
 * This allows subscribers to know when a publisher has
 * reset and no longer knows about the subscriber.
 *
 * The session-id for the Chariot connected to this Arduino
 * will be returned for a mote string of "".
 */
 int ChariotEPClass::getSessionId(String& mote)
 {
	String response, opts = "";
	String sessionId = "session-id";
	
    ChariotEP.coapRequest(COAP_GET, mote, sessionId, TEXT_PLAIN, opts, response);
    if (ChariotEP.strip_205_CONTENT(response))
    {
